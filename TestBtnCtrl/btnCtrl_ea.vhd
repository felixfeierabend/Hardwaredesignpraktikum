...
entity btnCtrl is
    port (
        ...
        inc_o : out std_ulogic;
		  dec_o : out std_ulogic
    );
end entity btnCtrl;
...
begin
    ...
    increment_s <= btnIncrement_i;
    decrement_s <= btnDecrement_i;
    switchTen_s <= switchTen_i;
        ...
        case fsm_state is
            ...
            when INCREMENT =>
                ...
                if (unsigned(adc_value) + incValue) > to_unsigned(ADC_MAX_VAL, ADC_BIT_WIDTH) then
                    next_adc_value <= std_ulogic_vector(to_unsigned(ADC_MAX_VAL, ADC_BIT_WIDTH));
                end if;
					 dec_o <= '0';
					 inc_o <= '1';
